describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const prodItems = await page.$$('product-item');
    let data=prodItems[0];
    let Shadow = await data.getProperty('shadowRoot');
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let data=prodItems[i];
    let Shadow = await data.getProperty('shadowRoot');
    let button = await Shadow.$('button');
    await button.click();
    }
    let cart = await page.$('#cart-count');
    let count = await cart.getProperty('innerText');
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    let cart_item = await page.evaluate(() => {
